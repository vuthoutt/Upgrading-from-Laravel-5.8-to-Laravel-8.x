...
class FixerSurveyStartDate extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:FixerSurveyStartDate';
    ...
    {
        $zones = Zone::whereIn('id',[2,4,7,8])->get();
    ...
    public function moveSurveyDocument($groupID) {
        $ids = "(813,814,815,818,819,821,822,823,824,825,826,827,828,829,830,831,832,833)";
        // sample certificate
        $sql_sample =  "SELECT `z`.zone_name,
                    ...
                    WHERE `z`.`id` = $groupID and sds.type = 'sct' and s.created_at < '2020-01-01 00:00:00' and s.id in $ids ";
        ...
        $sql_plan =  "SELECT `z`.zone_name,
                        ...
                        LEFT JOIN tbl_survey_type st ON `st`.`id` = `s`.`survey_type`
                        LEFT JOIN tbl_shine_document_storage sds on sds.object_id = sc.id
                        WHERE `z`.`id` = $groupID and sds.type = 'p' and s.created_at < '2020-01-01 00:00:00' and s.id in $ids";
        ...
        $sql_survey =  "SELECT `z`.zone_name,
                        ...
                        WHERE `z`.`id` = $groupID and s.created_at < '2020-01-01 00:00:00' and s.id in $ids";
