@extends('layouts.app')
@section('content')
@include('partials.nav', ['breadCrumb' => 'survey_add','color' => 'orange', 'data' => $property])

<div class="container prism-content">
    <h3>Add Survey</h3>
        <div class="main-content">
        <form method="POST" action="{{ route('survey.post_add',['property_id' => $property_id]) }}" id="form_add_survey" enctype="multipart/form-data" class="form-shine">
            @csrf
            <input type="hidden" name="list_area[]">
            <input type="hidden" name="list_location[]">
            <input type="hidden" name="list_item[]">
            <div class="row register-form">
                <label class="col-md-3 col-form-label text-md-left font-weight-bold" >Survey Type:
                    <span style="color: red;">*</span>
                </label>

                <div class="col-md-5">
                    <div class="form-group ">
                        <select class="form-control @error('surveyType') is-invalid @enderror" name="surveyType" id="surveyType">
                            <option value=""></option>
                            ...
                            {{-- <option value='5'>No ACM Management Survey</option> --}}
                        </select>
                        @error('surveyType')
            ...
            @include('forms.form_datepicker',['title' => 'Survey Date:', 'data' => date('d/m/Y'), 'name' => 'date' ])
            @include('forms.form_datepicker',['title' => 'Survey Due Date:', 'data' => date('d/m/Y'), 'name' => 'due-date' ])
            @include('forms.form_datepicker',['title' => 'Survey Start Date:', 'data' => date('d/m/Y'), 'name' => 'sv-start-date' ])
            @include('forms.form_datepicker',['title' => 'Survey Finish Date:', 'data' => date('d/m/Y'), 'name' => 'sv-finish-date' ])
            ...
            @include('forms.form_input',['title' => 'Survey Cost:', 'data' => '', 'name' => 'cost', 'maxlength' => 200])
            @include('forms.form_input',['title' => 'Organisation Reference:', 'data' => '', 'name' => 'organisation_reference', 'maxlength' => 200])

            @include('forms.form_dropdown',['title' => 'Asbestos Lead:', 'data' => $surveyUsers, 'name' => 'leadBy', 'key'=> 'id', 'value'=>'full_name', 'required' => true, 'compare_value' => old('leadBy') ?? '' ])
            @include('forms.form_dropdown',['title' => 'Second Asbestos Lead:', 'data' => $surveyUsers, 'name' => 'secondLeadBy', 'key'=> 'id', 'value'=>'full_name', 'compare_value' => old('secondLeadBy') ?? '' ])

            @include('forms.form_select_ajax',['title' => 'Surveyor:', 'name' => 'surveyor' ])
            @include('forms.form_select_ajax',['title' => 'Second Surveyor:', 'name' => 'secondSurveyor' ])
            @include('forms.form_select_ajax',['title' => 'Project Manager:', 'name' => 'consultantKey' ])
            <div class="row register-form">
                        ...
                        <select  class="form-control @error('projectKey') is-invalid @enderror" name="projectKey" id="projectKey">
                            ...
                            <option value="-1">------ Please select a project -------</option>
                                @foreach($projects as $project)
                                    <option value="{{ $project->id }}" {{(old('projectKey') ?? '') == $project->id ? 'selected' : '' }}>{{ $project->reference }}</option>
                                @endforeach
                            @else
            ...
            @include('forms.form_checkbox_survey',['title' => 'Settings:', 'name' => 'priorityAssessmentRequired','id' => 'priority_ass','title_right' => 'Priority Risk Assessment (PAS)', 'data' => '','compare' => 1 ])
            @include('forms.form_checkbox_survey',['name' => 'constructionDetailsRequired','id' => 'construction_detail','title_right' => 'Property Construction Details','data' => '','compare' => 1 ])
            @include('forms.form_checkbox_survey',['name' => 'locationVoidInvestigationsRequired','id' => 'location_void','title_right' => 'Location Void Investigations','data' => '','compare' => 1 ])
            @include('forms.form_checkbox_survey',['name' => 'locationConstructionDetailsRequired','id' => 'location_con','title_right' => 'Location Construction Details','data' => '','compare' => 1 ])
            @include('forms.form_checkbox_survey',['name' => 'RDinManagementAllowed','id' => 'r_and_d','title_right' => 'R&D Elements','data' => '','compare' => 1 ])
            @include('forms.form_checkbox_survey',['name' => 'licenseStatusRequired','id' => 'license_status','title_right' => 'Licensed/non Licensed','data' => '','compare' => 1 ])
            @include('forms.form_checkbox_survey',['name' => 'photosRequired','id' => 'photo_re','title_right' => 'Item Photos Required','data' => '','compare' => 1 ])
            @include('forms.form_checkbox_survey',['name' => 'propertyPlanPhoto','id' => 'property_plan','title_right' => 'Surveyors Notes Required','data' => '','compare' => 1 ])
                                    ...
                                    <input id="bstree-data" type="hidden" name="bstree-data" data-ancestors="">
                                        <li data-id="{{ $area['area']->id }}" data-area-id="{{ $area['area']->id }}" data-level="area"><span class="font-weight-bold" >{{ $area['area']->title }}</span>
                                            @if(isset($area['locations']))
                                                @include('surveys.area_location',['data' => $area['locations'] ])
                                            @endif
                                        </li>
                                    @endforeach
                                </ul>
                    ...
                    if (html) {
                        ...
                        $.each( html, function( key, value ) {
                            $('#surveyor').append($('<option>', {
                                ...
                                selected : '{{old('surveyor')}}' == value.id
                            }));

                            $('#secondSurveyor').append($('<option>', {
                                value: value.id,
                                text : value.first_name + ' ' + value.last_name,
                                selected : '{{old('secondSurveyor')}}' == value.id
                            }));

                            $('#qualityKey').append($('<option>', {
                                value: value.id,
                                text : value.first_name + ' ' + value.last_name,
                                selected : '{{old('qualityKey')}}' == value.id
                            }));

                            $('#analystKey').append($('<option>', {
                                value: value.id,
                                text : value.first_name + ' ' + value.last_name,
                                selected : '{{old('analystKey')}}' == value.id
                            }));

                            $('#consultantKey').append($('<option>', {
                                value: value.id,
                                text : value.first_name + ' ' + value.last_name,
                                selected : '{{old('consultantKey')}}' == value.id
                            }));
                        });
                    }
                }
