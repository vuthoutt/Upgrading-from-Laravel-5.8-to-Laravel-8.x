...
@push('javascript')
    <script type="text/javascript">
        ...
        $('#{{ isset($modal_id) ? $modal_id : '' }}').on('show.bs.modal', function(e) {
            ...
            if (type == 'cancel') {
                $('#title-ques-incident-{{ $type }}').html('Are you sure that you want to cancel incident report ' + incident_ref + ' ?');
                $('#form-cancel-{{ $type }}').attr('action', '/compliance/incident_reporting/cancel/' + incident_id);
            }
        });
    </script>
@endpush
