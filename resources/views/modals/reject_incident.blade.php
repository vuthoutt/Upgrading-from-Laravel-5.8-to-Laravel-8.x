...
<div class="modal fade" id="{{ isset($modal_id) ? $modal_id : '' }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="" method="POST" class="form-shine" id="form-incident-{{ $type }}">
                @csrf
                <div class="modal-body" style="min-height:140px;padding-left: 45px;padding-top: 50px">
                    <strong id="title-reject-incident" ></strong>
                    ...
                    <div class="row register-form">
                        ...
                        <div class="col-md-5">
                            <div class="form-group ">
                                <input class="form-control" name="due_date" id="incident_due_date" width="276" value="{{ date('d/m/Y') }}">
                            </div>
                        </div>
                    </div>
...
$('#{{ isset($modal_id) ? $modal_id : '' }}').on('show.bs.modal', function(e) {
    $('#incident_due_date').datepicker({
        uiLibrary: 'bootstrap4',
        format: 'dd/mm/yyyy',
        todayHighlight: true
    });

    var incident_id = $(e.relatedTarget).data('incident-id');
    var incident_ref = $(e.relatedTarget).data('incident-ref');
    // var due_date = $(e.relatedTarget).data('due-date');
    var type = $(e.relatedTarget).data('type');

    $('#title-reject-incident').html('Are you sure that you want to reject incident report ' +  incident_ref + ' ?');
    // $('#survey_due_date').val(due_date);
    $('#form-incident-{{ $type }}').attr('action', '/compliance/incident_reporting/reject/' + incident_id);
