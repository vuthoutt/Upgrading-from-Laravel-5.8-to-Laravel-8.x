        ...
        <div class="modal-content">
            ...
            <form >
                ...
                <div class="mb-3" style="text-align: center;">
                    ...
                    <button class="btn light_grey_gradient shine_submit_modal" id="submit_area">Submit</button>
                </div>
            </form>
        </div>
    </div>
       ...
       $.ajax({
            ...
            success: function(data){
                  if(data.errors) {
                      ...
                      if (survey_id == 0) {
                        location.href = '/property/detail/' + property_id + '?section={{ SECTION_AREA_FLOORS_SUMMARY }}&area=' + data.id +'&position={{$position ?? 0}}&category={{$category ?? 0}}';
                      } else {
                        location.href = '/surveys/' + survey_id + '?section={{ SECTION_AREA_FLOORS_SUMMARY }}&area=' + data.id +'&position={{$position ?? 0}}&category={{$category ?? 0}}';
                      }
                  } else {
