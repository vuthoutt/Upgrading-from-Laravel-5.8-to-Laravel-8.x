    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header {{ isset($color) ? CommonHelpers::getNavColor($color) : 'red_gradient' }}">
                <h5 class="modal-title" id="exampleModalLabel">shinePrism - {{ ($action == 'add') ? 'Add' : 'Edit' }} Work Supporting Document</h5>
            ...
            <meta name="csrf-token" content="{{ csrf_token() }}" />
                <div class="modal-body" style="padding-left: 20px;min-height:190px">
                    ...
                    <input type="hidden" class="form-control" name="work_id" id="work_id" value="{{ $work_id ?? '' }}">
                    @if($action == 'edit')
                        <input type="hidden" class="form-control" name="id" id="data_id" value="{{ isset($data) ? $data->id : '' }}">
                    @endif
                    @include('forms.form_input',['title' => 'Document Name:', 'name' => 'name','id' => 'form-name', 'width_label' => 4, 'width' => 7 , 'data' => $data->name ?? '', 'required' => true])
                    ...
                    <div class="form-group row {{ ($action == 'add') ? 'mt-4' : 'mt-2' }}">
                        ...
                        <input class="col-md-8" type="file" class="form-control-file" id="form-document" name="document" value="{{ !($action == 'add') ? CommonHelpers::getFile($data->id, WORK_REQUEST_FILE) : '' }}">
