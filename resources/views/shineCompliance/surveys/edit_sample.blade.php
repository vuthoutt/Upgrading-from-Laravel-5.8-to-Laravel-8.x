...
@include('partials.nav', ['breadCrumb' => 'sample_compliance', 'data' => $breadcrumb_data, 'color' => 'orange'])
<div class="container prism-content">
    <h3>Sample {{ $sample->description }} - {{ $sample->reference }}</h3>
    <div class="main-content">
    <form method="POST" action="{{ route('shineCompliance.sample_postEdit',['survey_id' => $survey_id, 'sample_id' => $sample->id]) }}" class="form-shine">
        @csrf
        <input type="hidden" name="pagination_type" value="{{ TYPE_SAMPLE }}">
        <input type="hidden" name="position" value="{{ $position }}">
        @include('shineCompliance.forms.form_text',['title' => 'Survey ID:', 'data' => $survey_id ])
        @include('shineCompliance.forms.form_input',['title' => 'External Reference:', 'name' => 'reference','data' => $sample->description, 'required' => true ])
                ...
                <div class="form-group">
                    @if(!empty($sample->item_reference) and !empty($sample->item_ids))
                         @foreach($sample->item_reference as $key => $item_reference)
                             <a href="{{ route('shineCompliance.item.detail', ['id' =>  $sample->item_ids[$key]]) }}">{{ $item_reference }}</a>
                              @include('shineCompliance.forms.form_input_hidden',['name' => 'list_item_id[]', 'data' => $sample->item_ids[$key] ])
                             @if(!$loop->last)
                                 ,
                             @endif
                         @endforeach
        ...
        <div id="asbestos-fibre">
            @include('shineCompliance.forms.form_dropdown',['title' => 'Asbestos Fibre (d)', 'data' => $assessmentAsbestosKeys, 'name' => 'assessmentAsbestosKey', 'key'=> 'id', 'value'=>'description', 'option_data' => 'score', 'compare_value' => $selectedAssessmentAsbestosKeys ])
        </div>
        <div class="btn" >
