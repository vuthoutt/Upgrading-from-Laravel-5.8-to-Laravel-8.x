...
@section('content')
@include('shineCompliance.partials.nav', ['breadCrumb' => $breadcrumb_name,'color' => 'orange', 'data' => $breadcrumb_data])

<div class="container-cus prism-content">
    <div class="row">
        <h3 class="title-row">{{ $survey->property->name }} {{ $survey->reference }}</h3>
    </div>
    <div class="main-content">
        ...
        <div class="tab-content">
            <div id="Details" class="container tab-pane active">
                @include('shineCompliance.surveys.detail',['data' => $survey])
            </div>
            ...
            <div id="Assessments" class="container tab-pane fade">
                @if($type)
                    ...
                    @else
                    @include('shineCompliance.properties.item_summary')
                    @endif
                @else
                    @switch($section)
                        @case(SECTION_AREA_FLOORS_SUMMARY)
                            @include('shineCompliance.surveys.area',['areaData'=> $areaData, 'areas' => $dataTab, 'decommissionedAreas' => $dataDecommisstionTab,'survey' => $survey, 'surveyAssessment' => $dataSummary])
                        @break
                        @case(SECTION_ROOM_LOCATION_SUMMARY)
                            @include('shineCompliance.surveys.location',['locationData'=> $locationData, 'survey' => $survey, 'surveyAssessment' => $dataSummary])
                        @break
                        @default
                            @include('shineCompliance.surveys.assessments',[ 'areas' => $dataTab, 'decommissionedAreas' => $dataDecommisstionTab,'survey' => $survey, 'surveyAssessment' => $dataSummary])
                    @endswitch
                @endif
            </div>

            <div id="Samples" class="container tab-pane fade">
                @include('shineCompliance.surveys.samples',['data' => $survey])
            </div>
            <div id="executive-summary" class="container tab-pane fade">
                @include('shineCompliance.surveys.executive_summary',['data' => $survey])
            </div>
            <div id="objective-scope" class="container tab-pane fade">
                @include('shineCompliance.surveys.objectives_scope',['data' => $survey])
            </div>
            <div id="limitations" class="container tab-pane fade">
                @include('shineCompliance.surveys.limitations',['data' => $survey])
            </div>
            <div id="method" class="container tab-pane fade">
                @include('shineCompliance.surveys.method',['data' => $survey])
            </div>
            <div id="air-test" class="container tab-pane fade">
                @include('shineCompliance.surveys.air_test_certificate',['survey' => $survey])
            </div>
            <div id="plans" class="container tab-pane fade">
                @include('shineCompliance.surveys.plans',['surveyorsNotes' => $surveyorsNotes, 'plans' => $plans])
            </div>
            <div id="property-information" class="container tab-pane fade">
                @include('shineCompliance.surveys.property_information',['data' => $survey])
            </div>
        </div>
    </div>
</div>
...
$(document).ready(function(){
        ...
        function auditTab(tab) {
            $.ajax({
                type: "POST",
                url: "{{route('shineCompliance.ajax.ajax_audit')}}",
