...
@section('content')
    @if($assess_id == 0)
        @include('shineCompliance.partials.nav',['breadCrumb' => 'register_vehicle_parking_add', 'color' => 'red', 'data'=> $property ?? []])
    @else
        @include('shineCompliance.partials.nav',['breadCrumb' => 'property_assessment_vehicle_parking_add', 'color' => 'orange', 'data'=> $assessment ?? []])
    @endif
    <div class="container-cus prism-content pad-up">
        <div class="row">
            <h3 class="title-row">Add Vehicle Parking</h3>
        </div>
        <div class="main-content">
            <form method="POST" action="{{ route('shineCompliance.assessment.post_add_vehicle_parking')}}" enctype="multipart/form-data" class="form-shine">
                @csrf
                ...
                @include('shineCompliance.forms.form_input',['title' => 'Vehicle Parking Name:','name' => 'name','required' => true])
                ...
                @include('shineCompliance.forms.form_upload',['title' => 'Vehicle Parking Photo:', 'name' => 'photo'])
                @include('shineCompliance.forms.form_text_area', ['title' => 'Vehicle Parking Comments:', 'name' => 'comment'])
                <a href="">
