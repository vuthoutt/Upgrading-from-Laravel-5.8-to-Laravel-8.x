...
@section('datatable_content')
    @if(isset($data) and count($data) > 0)
        <form method="POST" action="" id="form-sample">
        <meta name="csrf-token" content="{{ csrf_token() }}" />
        <input type="hidden" name="data-checked">
        <input type="hidden" name="data-unchecked">
        @foreach($data as $key => $dataRow)
                <tr>
                    <td><a href="{{ route('shineCompliance.sample_getEdit',['survey_id' => $survey->id, 'sample_id' => $dataRow->sample_id,'position' => $key, 'pagination_type' => TYPE_SAMPLE]) }}">{{ $dataRow->reference }}</a></td>
                    <td style="width: 140px"><a href="{{ route('shineCompliance.sample_getEdit',['survey_id' => $survey->id, 'sample_id' => $dataRow->sample_id,'position' => $key, 'pagination_type' => TYPE_SAMPLE]) }}">{{ $dataRow->description }}</a></td>
                    <td>
                        @if(!empty($dataRow->item_reference) and !empty($dataRow->item_ids))
                            @foreach($dataRow->item_reference as $key => $item_reference)
                                <a href="{{ route('shineCompliance.item.detail', ['id' =>  $dataRow->item_ids[$key]]) }}">{{ $item_reference }}</a>
                                @if(!$loop->last)
                                    ,
                                @endif
                            @endforeach
...
$(document).ready(function(){
    ...
    $("#save-sample1").click(function(){
        ...
        $.ajax({
          type: 'POST',
          url : "{{ route('shineCompliance.ajax.pick_sample') }}",
