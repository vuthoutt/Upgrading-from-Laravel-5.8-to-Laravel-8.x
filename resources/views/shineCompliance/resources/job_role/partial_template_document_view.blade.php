                ...
                <ul>
                    @foreach($t->template as $document)
                        <li data-zone="list-zone" data-id="child{{$prefix}}-document-{{$document->id}}"
                            data-checked="{{in_array($document->id, $checked_array) ? 1 : 0}}"
                            data-template-child="1" data-value="{{$document->id}}" data-checked="0"
                            data-level="{{$level + 2}}" data-parent="document-{{$prefix}}"  data-prefix = "{{$prefix}}">
                            <span>{{$document->name}}
                            </span>
                        </li>
                    @endforeach
