...
<div class="modal fade" id="{{ isset($modal_id) ? $modal_id : '' }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form method="POST" id="form-cancel-assess-{{ $type }}" class="form-shine">
            @csrf
            <div class="modal-body" style="height:140px;padding-left: 80px;padding-top: 50px">
                <strong id="title-ques-assess-{{ $type }}" ></strong>
...
$('#{{ isset($modal_id) ? $modal_id : '' }}').on('show.bs.modal', function(e) {
    var assess_id = $(e.relatedTarget).data('assessment-id');
    var assess_ref = $(e.relatedTarget).data('assessment-ref');
    var type = $(e.relatedTarget).data('type');

    if (type == 'cancel') {
        $('#title-ques-assess-{{ $type }}').html('Are you sure that you want to cancel assessment ' + assess_ref + ' ?');
        $('#form-cancel-assess-{{ $type }}').attr('action', '/compliance/assessment/' + assess_id + '/cancel');
    }
});
</script>
@endpush
