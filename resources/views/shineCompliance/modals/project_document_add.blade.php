    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body" style="padding-left: 30px;min-height:140px ">
                ...
                @include('shineCompliance.form_search.search_document',['title' => 'Document Title:', 'name' => 'name','id' => 'form-name'.$contractor_key , 'width' => 7,'width_label' => 4 , 'data' => '', 'required' => true, 'required_html' => 'required'])
                <span class="invalid-feedback font-weight-bold modal-err-text" role="alert" id="name-err{{ $contractor_key ?? '' }}"></span>
                @include('shineCompliance.forms.form_dropdown',['title' => 'Document Type:', 'data' => $doc_types ,'width_label' => 4, 'width' => 7, 'name' => 'type', 'key'=> 'id', 'value'=>'doc_type', 'compare_value' => '' ])
                ...
                <div class="row register-form">
                    <label class="col-md-4 font-weight-bold" for="document_file">Document: <span style="color: red;"> *</span></label>
                    <input class="col-md-8" type="file" class="form-control-file" id="form-document_file{{ $contractor_key ?? '' }}" name="document_file" value="" required>
                    <span class="invalid-feedback font-weight-bold modal-err-text" role="alert" id="document_file-err{{ $contractor_key ?? '' }}"></span>
                </div>
                 @if( (\Auth::user()->client_type == 0 || \Auth::user()->client_id == $project->client_id) and ( in_array($doc_cat, [TENDER_DOC_CATEGORY]) ) )
                    @include('shineCompliance.forms.form_datepicker',['title' => 'Deadline Date:', 'name' => 'deadline', 'width_label' => 4, 'id' => 'deadline'.( $contractor_key ?? '') ])
                @endif
            ...
            <div class="mb-4" style="text-align: center;">
                ...
                <button id="add_document_{{ $contractor_key ?? '' }}" class="btn light_grey_gradient shine_submit_modal fs-8pt" >Add</button>
            </div>
            </form>
        </div>
    </div>
