...
@section('content')
    ...
    @include('shineCompliance.partials.nav',['breadCrumb' => 'system_document_compliance', 'color' => 'red', 'data' => $object])
    <div class="container-cus prism-content pad-up">
        ...
        <div class="main-content mar-up">
            @include('shineCompliance.properties.partials._property_button_search',
                    [ 'backRoute' =>  route('shineCompliance.property.property_detail',['zone_id' => $object->property_id ?? 0 ]),
                        'addRoute' => $can_add_new ? '#property-category-add' : false,"class_css" => "-25px"
                    ])
            <div class="row">
                @include('shineCompliance.properties.partials._property_aov_system_sidebar',[
                    'image' =>  asset(\ComplianceHelpers::getSystemFile($object->id, COMPLIANCE_SYSTEM_PHOTO)),
                    'system_id' => $object->id ?? 0,
                    'route_document' => route('shineCompliance.system.document.list', ['id'=>$object->id ?? 0, 'type'=> DOCUMENT_SYSTEM_TYPE])
                    ])
                    ...
                    <div class="property-detail">
                        <div class="row">
                            <div class="col-12 mb-1 pr-0">
                                <input type="hidden" class="sys_id" name="sys_id" value="{{$object->id}}">
                        ...
                        'property_id' => $object->property_id,'system_id' => $object->id
                        ])
                    </div>
                </div>
            </div>
                        ...
                        $.ajax({
                            ...
                            success: function (html) {
                                ...
                                $.each( html, function( key, value ) {
                                    console.log(value.id,default_id)
