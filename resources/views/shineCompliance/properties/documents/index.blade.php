...
@section('content')
    @if(isset($property->parents))
        @include('shineCompliance.partials.nav',['breadCrumb' => 'sub_property_document_compliance', 'color' => 'red', 'data' => $property ?? ''])
    @else
        @include('shineCompliance.partials.nav',['breadCrumb' => 'property_document_compliance', 'color' => 'red', 'data' => $property ?? ''])
    @endif
    ...
    <div class="container-cus prism-content pad-up" >
        ...
        <div class="main-content mar-up">
            @if(isset($property->parents))
                @include('shineCompliance.properties.partials._property_button_search',['backRoute' => route('shineCompliance.property.property_detail',
                ['id' => $property->parent_id]),'addRoute' => $can_add_new ? '#property-category-add' : false,"class_css" => "0px"])
            @else
                @include('shineCompliance.properties.partials._property_button_search',['backRoute' => route('shineCompliance.property.property_detail',
                ['id' => $property->id]),'addRoute' => $can_add_new ?  '#property-category-add' : false,"class_css" => "0px"])
                @endif
...
{{--                            @include('shineCompliance.properties.documents.system_document_tab', ['data' => $system])--}}
{{--                        @endforeach--}}
{{--                        @if(count($documents_no_parent) > 0)--}}
{{--                            @include('shineCompliance.properties.documents.no_document_tab', ['data' => $documents_no_parent])--}}
{{--                        @endif--}}
                    </div>
                    <!-- modal category -->
                    ...
                    @include('shineCompliance.modals.property_category_edit',[
                                'modal_id' => 'property-category-edit',
                                'property_id' => $property->id])
                    <!-- modal document -->
                    @include('shineCompliance.modals.property_document_add',[
                            'color' => 'red',
    ...
    <script>
        $(document).ready(function(){
            ...
            $("#property-document-edit").on("show.bs.modal", function (e) {
                ...
                // var modal_category_id = $(e.relatedTarget).data('modal_category_id');
                ...
                if(parent_type == 2 || parent_type == 3){//2 system, 3 equipment, 4 programme
                    ...
                    //     $(that_parent).closest('.modal').find('.property_programme').show();
                        ...
                        if(system_id > 0){
                            ajaxProgramme(that, property_id, system_id, programme_id);
                        }
            ...
            $("#property-category-edit").on("show.bs.modal", function (e) {
                var that = $(this);
                var url = $(e.relatedTarget).data('url');
                var id = $(e.relatedTarget).data('id');
                var name = $(e.relatedTarget).data('name');
                $(that).closest('.modal').find('form').attr('action', url);
                $(that).closest('.modal').find('input[name="category_id"]').val(id);
                $(that).closest('.modal').find('input[name="category_title"]').val(name);
            });
            ...
            $("#property-document-add").on("show.bs.modal", function (e) {
                ...
                if(!category_id){
                    var that = $(this);
                    var parent_type = 2;//default selected equipment
                    var system_id = $(e.relatedTarget).data('system-id');
                    var equipment_id = '';
                    var programme_id = '';
            ...
            $('body').on('click', '.shine_document_submit', function(e){
                ...
                var arr_validate = ['.parent_type','.property_system','.property_equipment',
                    'input[name="category_title"]','input[name="name"]','select[name="type[]"]','select[name="category_id"]',
                    'input[name="date"]','input[name="document"]', 'select[name="document_status"]', 'input[name="enforcement_deadline"]'];
                ...
                if(array_new_type.includes($(that).closest('form').find('.child-document-type').val())){
                    arr_validate = ['.parent_type','.property_system','.property_equipment',
                        'input[name="category_title"]','input[name="name"]','select[name="type[]"]','select[name="category_id"]',
                        'input[name="date"]','input[name="document"]'];
                }
            ...
            //historical submit
            $('body').on('click', '.shine_historical_document_submit', function(e){
                e.preventDefault();
                var that = this;
                var is_error = false;
                var arr_validate = ['input[name="name"]','select[name="document_type"]',
                    'select[name="category"]','input[name="document"]'];
                if($(that).closest('form').find('.document_id').val() > 0){
                    var index = arr_validate.indexOf('input[name="document"]');
                    if (index !== -1) {
                        arr_validate.splice(index, 1);
                ...
                if(!is_error){  //send request
                    console.log(is_error, $(that).closest('form').serialize());
                    // return;
                    $(that).closest('form').submit();
                }
            });
            ...
            $('body').on('change', '.document_type', function(){
                var is_extend_ms = $(this).find('option:selected').data('option');
                var text = $(this).find('option:selected').text();
                console.log(is_extend_ms);
                if($(this).closest('form').find('.is_external_ms').length){
                    if(is_extend_ms == 1 || is_extend_ms == 2 || text == 'Asbestos Refurbishment Survey'){
                        $(this).closest('form').find('.boolean_switches').show();
                    } else {
                        $(this).closest('form').find('.boolean_switches').hide();
                    }
                } else {
                    console.log(is_extend_ms, text, $(this).closest('.modal-content').find('.boolean_switches'))
                    if(is_extend_ms == 1 || is_extend_ms == 2 || text == 'Asbestos Refurbishment Survey'){
                        $(this).closest('.modal-content').find('.boolean_switches').show();
                    } else {
                        $(this).closest('.modal-content').find('.boolean_switches').hide();
                    }
                    console.log($(this).closest('.modal-content').find('.is_external_ms'));
                }
            });
            ...
            //default will hide switch boolean for add modal
            //check value for edit modal
            $('.boolean_switches').hide();
            $('body').on('change', '.document_type', function(){
                var is_extend_ms = $(this).find('option:selected').data('option');
                var text = $(this).find('option:selected').text();
                if($(this).closest('form').find('.is_external_ms').length){
