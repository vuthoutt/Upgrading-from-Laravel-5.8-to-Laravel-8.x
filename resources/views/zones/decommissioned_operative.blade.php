<div class="container">
    @if($decommissioned_properties)
        <div class="row">
            @php
                // key is change from where query in collection
                    $count = 0;
            @endphp
            @foreach($decommissioned_properties as $key => $property)
                <div class="col-3">
                    <div class="property-opt">
                        <a class="text-decoration-none" href="{{route('property_detail', ['id' => $property->id, 'section' => SECTION_DEFAULT])}}" >
                        ...
                        <div class="download-operative">
                            <a href="{{route('register.pdf',['type'=>PROPERTY_REGISTER_PDF,'id'=>$property->id])}}" class="btn btn-outline-secondary w-100 download-pdf-btn">
