CREATE TRIGGER trigger_item_pd_insert AFTER INSERT ON tbl_item_product_debris_type_value FOR EACH ROW BEGIN
...
CREATE TRIGGER trigger_item_pd_update AFTER UPDATE ON tbl_item_product_debris_type_value FOR EACH ROW BEGIN
REPLACE INTO tbl_item_product_debris_view SET item_id = NEW.item_id, product_debris = (SELECT
GROUP_CONCAT(
IF
( ip3.description IS NOT NULL, CONCAT( ip3.description, '---' ), '' ),
...
CREATE TRIGGER trigger_item_pd_delete AFTER DELETE ON tbl_item_product_debris_type_value FOR EACH ROW BEGIN
